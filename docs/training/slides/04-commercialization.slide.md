...
> **一句话**：从单机 dev 到 B2B API → 多租户 SaaS 的演进路线，**B2B API 4 周可商用**，SaaS 还有 6 项 SLA gap。
...
- ✅ DB `owner_user_id` 列 + IdentityMiddleware 三层透传
...
| Gap | 工作量 |
|-----|-------|
| 1. 定价模型（per-chapter，失败不计费） | 3 天 |
| 2. 多租户隔离（tenant_id + token 映射） | 5 天 |
| 3. SLA + 限流（60/h，5 concurrent） | 5 天 |
| 4. LLM provider fallback（router + circuit breaker） | 5 天 |
| 5. 版权合规（输入来源声明） | 2 周 |
| 6. 监控仪表盘（Helicone + Langfuse 合并） | 1 周 |
