...
## 5.1 新增：人工反馈回路（推荐接入签约卷流程）

当 30 章签约卷已经生成、Reader Panel / risk gate / AI 味诊断都跑完后，
**不要直接宣布“完成”**，而是补一层人工阅读反馈：

```mermaid
flowchart TD
    A[30章择优精修版] --> B[人工连续阅读]
    B --> C[填写反馈模板]
    C --> D[定位最弱章节 / 最像AI的片段 / 最有签约感的章节]
    D --> E[单章结构性重写 or 精修]
    E --> F[Reader Panel 复核]
    F --> G[保留更优版本]
```
...
- [`output/projects/algorithm-immortal/final-polished/30章签约卷人工反馈模板.md`](../../output/projects/algorithm-immortal/final-polished/30章签约卷人工反馈模板.md)

### 人工反馈这一层解决什么问题

| 机器层能发现 | 人工层更擅长发现 |
|---|---|
| 规则崩坏、人物 OOC、时间线问题 | 哪一章最像 AI、哪一章最有签约感 |
| 节奏过平、环境发白、对话工具化 | 哪里真正让人“想继续看”或“想跳过” |
| comfort 低于阈值 | 编辑感、番茄感、追读欲望 |

### 实战原则

1. **先让人完整读，再改机器分数低的章**
2. **如果人工反馈和 panel 冲突，优先看人工反馈**
3. **精修后不全量替换，而是逐章择优**
4. **优先改“签约影响最大”的章，而不是最低分章**
...
- 顶层入口层现也开始显式暴露 `external_runtime_executor_preview` 与 `runtime-executor-review-surface`，使 runtime gate → runtime preview 的最后一跳在 root registry 中完整可见
