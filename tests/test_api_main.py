...
def test_api_readme_mentions_whole_book_imitation_samples() -> None:
    ...
    assert 'curl -X POST "http://127.0.0.1:8000/api/whole-book-imitation-run"' in readme
...
def test_whole_book_imitation_run_endpoint_classifies_provider_billing_errors(monkeypatch, tmp_path) -> None:
    ...
    with factory() as session:
        branch_id = _seed_branch(session, tmp_path / "whole-book-api-billing.txt")
    ...
    payload = json.loads(body)
    assert payload["error_code"] == "provider_billing_limited"
...
def test_whole_book_imitation_run_request_sample_is_executable(monkeypatch, tmp_path) -> None:
    ...
    with factory() as session:
        branch_id = _seed_branch(session, tmp_path / "whole-book-api-request-sample.txt")
    ...
    status, body = _call_post_json("/api/whole-book-imitation-run", payload)
    assert status == "200 OK"
    result = json.loads(body)
    assert result["contract_version"] == "whole-book-imitation.v1"
